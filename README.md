...
* https://maimai.cn/web/feed_explore
...
2. https://github.com/limboy/iOS-Developer-Interview-Questions
3. https://draveness.me/guan-yu-xie-ios-wen-ti-de-jie-da/
...
* https / TLS
  ...
  SSL 剥离: 通过攻击 DNS 响应，攻击者可以将自己变成中间人。https --> http
...
* 多路io
    ...
    --
    poll 不再用 BitsMap 来存储所关注的文件描述符，取而代之用动态数组，
...
* rpc: RPC（Remote Procedure Call），又叫做远程过程调用。它本身并不是一个具体的协议，而是一种调用方式。
...
* +load() 方法的调用是在 main() 函数之前，并且不需要主动调用，
